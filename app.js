jQuery(function($) {
    ...
    data.forEach(function(pal, i) {
        pal.id = i;
        $palettes.append('<option value="' + pal.id + '">' + pal.name + '</option>');
    });
    ...
    function onPaletteSelect(e) {
        ...
        palette.colors.forEach(function(item, i) {
            item.id = i;
            var value = '<option value="' + item.id + 
    ...
    function onColorSelect(e) {        
        ...
        if (colorText && colorBg && typeof colorText.lvrNumeric === "number" && typeof colorBg.lvrNumeric === "number") {            
            ...
            if(value < 70) {
                $('#contrast').html('Контраст ' + value + '%<span style="margin-left: 5px;"> <img class="smile" src="/i/icon-sad.svg"></span>');
                ...
                $('#closestCaption').html('');
                $('#contrast').html('Контраст ' + value + '%<span style=\'margin-left: 5px;\'> <img class="smile" src="/i/icon-smile.svg"></span>');
                $('#comment').html('');
                $('#closestText').html('');
    ...
    function updateClosestColors(isText, colors) {
        ...
        $('.closest-caption').on('click', onClosestClick);
    }
